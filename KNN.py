    ...
    def run(self): 
        ...
        for p in range(len(self.X_test)):
            ...
            for q in range(len(self.X_train)):
                distance = self.distance(self.X_test[p], self.X_train[q])
                distances.append(distance)
            distancesk = np.argsort(distances)
            for m in range(self.k):
                distances2.append(self.y_train[distancesk[m]])
                if distances2[m] == '>50K':
                    lt = lt + 1
                else:
                    lat = lat + 1
